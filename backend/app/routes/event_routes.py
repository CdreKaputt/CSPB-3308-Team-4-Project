...
def edit_event_form(event_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            db.session.rollback()
            flash("Could not create event. Please try again.", "error")
            return render_template("edit_event.html", trip_id=trip_id)

@main_bp.route("/<int:event_id>/delete", methods=["GET", "POST"])
@required_logged_in
def event_delete(event_id):
    ...
    try:
        ...
        return render_template("events.trip_events", trip_id=trip_id)